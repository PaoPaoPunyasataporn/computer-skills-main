...
(function(){
  ...
  // Mid-test milestone cheering disabled — this is a graded test, the only
  // celebration is the certificate/applause at the very end on a pass.
  function poll(){return;}
  void poll; void praises; void gvar; void lastCorrect; void lastStreak;

  const resEl=document.getElementById('results');
  // Disabled: victory() calls dlf.bigCongrats() directly so the applause plays
  // ONLY on a pass. The results screen is also reused for the defeat screen,
  // so we must NOT auto-fire the congrats overlay/applause from here.
  function checkResults(){return;
    if(!resEl)return;
    ...
    if(vis){if(congShownFor!=='on'){congShownFor='on';
      ...
      dlf.bigCongrats({trophy:(t&&t.textContent.trim())||'🏆',title:(ti&&ti.textContent.trim())||'จบบทเรียนแล้ว!',
        sub:(su&&su.textContent.trim())||'น้องทำสำเร็จแล้ว เก่งมาก!',stars:3});}}else congShownFor=null;}
  // Observer/interval disabled — congrats is fired explicitly by victory().
  ...
  function dlfIsControl(el){
    ...
    if(el.closest('button,a[href],input,.opt-btn,.btn,.mode-card,.care-btn,.cc-btn,.tab,.id-option,.ans-btn,.flip-card,.start-btn,.next-btn,.choice,[role="button"],.nav-btn,.new-tab-btn,.browser-toolbar,.browser-titlebar,.tabs-container,.address-bar,[class*="toolbar"],[class*="titlebar"]')) return true;
    return false;
  }
  ...
  var DLF_NOSAY={progTag:1,secProgTag:1,phaseTag:1,bossLabel:1,certScore:1,certDate:1,resSub:1,dlfCongSub:1,dlfCongTitle:1,feedback:1};
  ...
  function readable(el){
    ...
    if(el.id && DLF_NOSAY[el.id]) return;            // dynamic counters/date/celebration text: value changes at runtime, skip
    if(el.classList && el.classList.contains('no-say')) return;
    if(el.closest && el.closest('.no-say')) return;   // .no-say applies to the whole subtree
    ...
    if(!/[\u0E00-\u0E7F]/.test(txt)) return;
    if(/^[^\u0E00-\u0E7F]*[\u0E00-\u0E7F ]{0,12}\d+\s*\/\s*\d+/.test(txt)) return; // bare "X/Y" counters
  ...
  setInterval(decorate,1800);decorate();
